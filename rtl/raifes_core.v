...
module raifes_core(
		 ...
		 output			   	                  dmi_dm_busy,           // DM busy signaling, if the DM needs more time, it can set this flag. Usually, the DM is clocked much faster (system clock) than the DTM (external JTAG clock), so this is never required.
		 
		 // Debug Module ports. Added for direct writes to caeco
		 input     [`HASTI_BUS_WIDTH-1:0]      caeco_rdata,
		 output    [`HASTI_BUS_WIDTH-1:0]      caeco_wdata,
		 output                                caeco_cmd,
		 output                                caeco_wen
);
...
// F.B reduced since pcpi_wr_cop is not used         
assign	pcpi_wr = pcpi_ready_mul_div ? pcpi_wr_mul_div : 1'b0;
...
assign	pcpi_rd = pcpi_ready_mul_div ? pcpi_rd_mul_div : 
// F.B reduced, since pcpi_ready_cop and pcpi_rd_cop are not used withou coprocessor
//                  pcpi_ready_cop ? pcpi_rd_cop : 
                  32'hdeadbeef;
                  
assign	pcpi_wait = pcpi_wait_mul_div; // | pcpi_wait_cop; F.B reduced, since pcpi_wait_cop is not used
assign	pcpi_ready =  pcpi_ready_mul_div; // | pcpi_ready_cop; F.B reduced, since pcpi_ready_cop is not used
